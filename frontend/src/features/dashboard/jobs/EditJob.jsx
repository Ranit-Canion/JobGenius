...
import useJobPost from "../../jobs/useJobPost";
...
function EditJob() {
  ...
  const { isPending, updateJob } = useUpdateJobPost();
  console.log("postObj->", jobpost);
  ...
  useEffect(
    function () {
      ...
      setExpDate(jobpost?.expirationDate);
      setJobType(jobpost?.jobType);
      setExeLevel(jobpost?.experienceLevel);
      setDescription(jobpost?.description);
      setSelectedSkills(
        (jobpost?.skillsRequired || []).map((skill) => ({
          value: skill,
          label: skill,
        }))
      );
  ...
  return (
    <>
      <h1 className="mb-12 text-2xl text-gray-700 font-bold">Edit Job !</h1>
  ...
  `}
          ...
          <button
            type="submit"
            className="py-3 px-6 mt-4 cursor-pointer bg-blue-500 text-white font-semibold rounded-lg hover:bg-blue-600 transition duration-300"
            disabled={isPending}
          >
