...
import { useDeleteJobAlert } from "./useDeleteJobAlert";
...
function JobAlertJobSingle({ jobalert }) {
  const { deleteJobAlert } = useDeleteJobAlert();
  return (
      ...
      <div className="grid grid-cols-[5fr_1fr_1fr_1fr] gap-4 px-4 py-3 ">
        <div className="flex ">
          ...
          <div className="flex gap-1.5 ml-[2rem]  flex-col ">
            <h1 className="text-xl font-medium">{jobalert?.title}</h1>
            <div className="flex gap-2 w-[20%]">
              <div className="flex gap-1 items-center justify-center text-gray-500">
                <BriefcaseIcon className="w-[1.2rem]" />
                <p>{jobalert?.companyName}</p>
              </div>
              <div className="flex gap-1 items-center justify-center text-gray-500">
                <MapPinIcon className="w-[1.2rem]" />
                <p>{jobalert?.location}</p>
              </div>
            </div>
          </div>
        </div>
        <div>{getFormattedExpDate(jobalert?.createdAt)}</div>
        <div>{getFormattedExpDate(jobalert?.expirationDate)}</div>
        <div className="flex gap-4 items-center">
          <div className="flex gap-3 mb-[2.2rem] ml-[1rem]">
            <Modal>
              <Menus>
                <Menus.Toggle id={jobalert?._id} />
                <Menus.List id={jobalert?._id}>
                  <Menus.Button icon={<EyeIcon />}>View</Menus.Button>
                  <Modal.Open opens="delete-jobalert">
                ...
                <Modal.Window name="delete-jobalert">
                  <DeleteConfirmation
                    onConfirmed={() => deleteJobAlert(jobalert?._id)}
...
export default JobAlertJobSingle;
