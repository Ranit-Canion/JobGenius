...
function WorkExperienceForm() {
  ...
  function handleSubmit(e) {
    ...
    const updatedWorkExperience = [...(profile?.workExperience || []), obj];
    updateData({ workExperience: updatedWorkExperience });
  }
  return (
    <div className="bg-gray-50 p-[2rem] rounded-2xl shadow-sm">
      <h1 className="text-[1.2rem] font-medium">Awards or Certificates</h1>
      <form onSubmit={handleSubmit}>
        <div className="flex gap-2">
          <div className="flex flex-col gap-3 w-1/2">
            <label>Award or Certificate Title</label>
            <input
              type="text"
      ...
      <div className="mt-[1rem] flex flex-col gap-[1rem]">
        {profile?.workExperience?.map((obj) => (
          <WorkExperienceSingle educationObj={obj} />
        ))}
      </div>
    </div>
  );
